...
export default function CustomerHomePage() {
  ...
  const fetchProducts = async (category = '') => {
    try {
      const response = await fetch(
        `https://salessavvy-backend-production.up.railway.app/api/products${category ? `?category=${category}` : '?category=Shirts'}`, 
  ...
  const fetchCartCount = async () => {
    ...
    try {
      const response = await fetch(`https://salessavvy-backend-production.up.railway.app/api/cart/items/count?username=${username}`, {
        credentials: 'include', // Include authToken as a cookie
      });
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      const response = await fetch('https://salessavvy-backend-production.up.railway.app/api/cart/add', {
        credentials: 'include',
        method: 'POST',
